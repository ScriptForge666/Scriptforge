...
#define _BASICJSON_TEMPLATE_DEFINE_ \
template< \
...
#define _BASICLANG_HEAD_ template<_BASICJSON_TEMPLATE_>
#define _BASICLANG_HEAD_DEFINE_ template<_BASICJSON_TEMPLATE_DEFINE_>
#define _BASICLANG_ BasicLang<ObjectType, ArrayType, StringType, BooleanType, NumberIntegerType, NumberUnsignedType, NumberFloatType, AllocatorType, JSONSerializer, BinaryType, CustomBaseClass>
...
namespace Scriptforge {
    inline namespace Local {
        ...
        export _BASICLANG_HEAD_DEFINE_
        class BasicLang {
        public:
            using json_t = nlohmann::basic_json<ObjectType, ArrayType, StringType, BooleanType, NumberIntegerType, NumberUnsignedType, NumberFloatType, AllocatorType, JSONSerializer, BinaryType, CustomBaseClass>;
            BasicLang(Scriptforge::LanguageCode::Language lang = Scriptforge::LanguageCode::Language::Neutral, fs::path p = { "./lang" });
            void reload();
            void setLocale(const Scriptforge::LanguageCode::Language lang);
            Scriptforge::LanguageCode::Language getLanguageCode() const;
            template<typename T>
            ...
            template<typename T>
                requires is_basic_string<T>
            const json_t atJ(const T& key) const;
            template<typename T>
                requires is_basic_string<T>
            T value(const T& key, const T& defaultValue) const;
            template<typename T, typename... Args>
            ...
            template<typename T>
                requires is_basic_string<T>
            std::vector<T> getKeys() const;
            const json_t& getJson() const;
            bool isLoaded() const;
            template<typename T>
                requires is_basic_string<T>
            json_t operator[](const T& key);
        private:
            void loadLanguageFile(Scriptforge::LanguageCode::Language lang, fs::path path);
            void LanguageIsLegal(Scriptforge::LanguageCode::Language lang) const;
            void pathIsLegal(const fs::path& path) const;
...
namespace Scriptforge {
    inline namespace Local {
        ...
        template<_BASICJSON_TEMPLATE_, typename T, typename... Args>
            requires is_basic_string<T>
        T format(const nlohmann::basic_json<_BASICJSON_TEMPLATE_>& j, const T& key, Args&&... args) {
        ...
        _BASICLANG_HEAD_
        _BASICLANG_::BasicLang(Scriptforge::LanguageCode::Language lang, fs::path p) {
            loadLanguageFile(lang, p);
        }

        _BASICLANG_HEAD_
        void _BASICLANG_::reload() {
            loadLanguageFile(m_lang, m_path);
        }


        _BASICLANG_HEAD_
        void _BASICLANG_::setLocale(const Scriptforge::LanguageCode::Language lang) {
            m_lang = lang;
            loadLanguageFile(lang, m_path);
        }
        ...
        _BASICLANG_HEAD_
        Scriptforge::LanguageCode::Language _BASICLANG_::getLanguageCode() const {
            return m_lang;
        }
        ...
        template<typename T>
            requires is_basic_string<T>
        T _BASICLANG_::getLanguageNameISO639_1() const {
            return str_convert<T, std::string>(Scriptforge::LanguageCode::ENUM_TO_ISO639_1().at(m_lang));
        }
        ...
        template<typename T>
            requires is_basic_string<T>
        T _BASICLANG_::getLanguageName() const {
            return str_convert<T>(m_j.value("language_name", Scriptforge::LanguageCode::ENUM_TO_ISO639_1().at(m_lang)));
        }


        _BASICLANG_HEAD_
        fs::path _BASICLANG_::getLangPath() const {
            return m_path;
        }
        _BASICLANG_HEAD_
        void _BASICLANG_::setLangPath(const fs::path& path) {
            m_path = path;
            loadLanguageFile(m_lang, path);
        }
        ...
        template<typename T>
            requires is_basic_string<T>
        T _BASICLANG_::at(const T& key) const {
            return str_convert<T, std::string>(m_j.at(str_convert<std::string, T>(key)));
        }

        _BASICLANG_HEAD_
        template<typename T>
            requires is_basic_string<T>
        const _BASICLANG_::json_t _BASICLANG_::atJ(const T& key) const {
            return _BASICLANG_::json_t(m_j.at(str_convert<std::string>(key)));
        }

        _BASICLANG_HEAD_
        template<typename T>
            requires is_basic_string<T>
        T _BASICLANG_::value(const T& key, const T& defaultValue) const {
            return str_convert<T, std::string>(m_j.value(str_convert<std::string, T>(key), str_convert<std::string, T>(defaultValue)));
        }
        ...
        template<typename T, typename... Args>
            requires is_basic_string<T>
        T _BASICLANG_::format(const T& key, Args&&... args) const {
            return m_j.format(key, std::forward<Args>(args)...);
        }
        ...
        template<typename T>
            requires is_basic_string<T>
        bool _BASICLANG_::has(const T& key) const {
            return m_j.contains(str_convert<std::string>(key));
        }
        ...
        template<typename T>
            requires is_basic_string<T>
        std::vector<T> _BASICLANG_::getKeys() const {
            const auto& obj = m_j.get_ref<const json_t::object_t&>();
        ...
        _BASICLANG_HEAD_
        const _BASICLANG_::json_t& _BASICLANG_::getJson() const {
            return m_j;
        }

        _BASICLANG_HEAD_
        bool _BASICLANG_::isLoaded() const {
            return !m_j.empty();
        }
        ...
        template<typename T>
            requires is_basic_string<T>
        _BASICLANG_::json_t _BASICLANG_::operator[](const T& key) {
            return _BASICLANG_::json_t(m_j[str_convert<std::string>(key)]);
        }


        _BASICLANG_HEAD_
        void _BASICLANG_::loadLanguageFile(Scriptforge::LanguageCode::Language lang, fs::path path) {
        ...
        _BASICLANG_HEAD_
        void _BASICLANG_::LanguageIsLegal(Scriptforge::LanguageCode::Language lang) const {
            if (lang == Scriptforge::LanguageCode::Language::Invariant) {
                throwErrWithoutJson(Scriptforge::ErrCode::ErrCode::LocalInvalidLanguageCode, __func__, "Invariant is not valid language codes for loading language files.");
            }
        }
        ...
        _BASICLANG_HEAD_
        void _BASICLANG_::pathIsLegal(const fs::path& filename) const {
            if (!fs::exists(filename)) {
                throwFileNotFoundError(filename, m_lang, __func__);
            }
            if (!fs::is_regular_file(filename)) {
